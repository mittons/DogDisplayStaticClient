...
const PORT = 8401; // Port for the proxy server
...
const modifyResponseHeadersMiddleware = async (req, res, next) => {
    // Function to capture the original send function
    const originalSend = res.send;

    // Modify res.send to manipulate response headers
    res.send = function(data) {
        // Modify headers here
        res.set('X-Custom-Signature-Header', 'awehhwaaawdxdc');

        // Call the original send function with the unmodified data
...
app.get('/renderBreeds', modifyResponseHeadersMiddleware, (req, res) => {
